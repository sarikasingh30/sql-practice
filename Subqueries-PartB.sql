...
SELECT product_id, COUNT(*) AS number_of_orders FROM sale GROUP BY product_id ORDER BY number_of_orders DESC LIMIT 3;


/* Problem 4 : The following query is given:
SELECT product_id FROM sale GROUP BY product_id ORDER BY COUNT(*) DESC LIMIT 3;
Display all data from the product table about the products returned by the 
above query */

-- SELECT * from product WHERE product_id IN ( SELECT product_id FROM sale GROUP BY product_id ORDER BY COUNT(*) DESC LIMIT 3);
...
-- SELECT product_id FROM sale GROUP BY product_id HAVING COUNT(*)=1;
 SELECT * from product WHERE product_id IN (SELECT product_id FROM sale GROUP BY product_id HAVING COUNT(*)=1);
...
SELECT product_name, list_price from product WHERE product_id IN (SELECT product_id FROM sale GROUP BY product_id HAVING COUNT(*) = 1)
