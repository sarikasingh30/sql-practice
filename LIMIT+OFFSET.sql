...
/*
Problem 1 : The SQL code that creates the tables for this exercise is in the file create.sql.
Create a query that displays the first ten rows from the job table.
*/
 -- SELECT * FROM job LIMIT 10

/*
Create a query that displays the top five highest paying positions 
(take into account the max_salary column).
*/

 -- SELECT job_title FROM job ORDER BY max_salary DESC LIMIT 5
...
*/
 ALTER TABLE job
 ADD COLUMN avg_salary REAL;
 
 UPDATE job
 set avg_salary = (min_salary + max_salary) / 2.0;
 SELECT * FROM job ORDER by avg_salary DESC limit 5;
...
*/
 ...
 SELECT * FROM job ORDER by avg_salary desc limit 1 offset 3;
